...
# **********************************************************************************************************************
id_columns <- c("STN", "YYYYMMDD", "date", "doy", "year", "month", "week", "day")

default_set_of_columns <- c(id_columns, "FG", "TG", "TN", "TX", "SQ", "SP", "Q", "RH", "NG")

dd_subset <- subset_data(dd)

expect_true(all(colnames(dd_subset) %in% default_set_of_columns), info = "test default subset of columns")
...
specific_set_of_columns <- c("FG", "TG", "TN", "TX")

dd_subset <- subset_data(dd, variables = specific_set_of_columns)

expect_true(
  all(colnames(dd_subset) %in% c(id_columns, specific_set_of_columns)),
  info = "test specific subset of default columns"
)

# columns not in defaul set
specific_set_of_columns <- c("PG", "PX", "UX", "EV24")
...
new_from_year <- "2010"
dd_subset <- subset_data(dd, startyear = new_from_year)

expect_equal(min(dd_subset$year), as.numeric(new_from_year), info = "test specific from-year")
