...
#' @export
#'
find_nearest_KNMI_station <- function(lat, lon, active = TRUE, temperature_sensor = TRUE) {
  ...
  location <- data.frame(lon = lon, lat = lat)

  # load data of KNMI-stations
  utils::data("stations")

  distance.to.stations <- stations %>%
...
#' @export
#'
find_nearest_KNMI_station2 <- function(location, active = TRUE, temperature_sensor = TRUE) {
  ...
  return(find_nearest_KNMI_station(location$lat, location$lon, active, temperature_sensor))
}
