...
#
set_column_names <- function(x, col.from, col.to, allow.absent.cols = TRUE) {
...
# voor get_6day_weather_prediction & get_14day_weather_prediction
extraheer_numerieke_waarden <- function(html_node) {
  text <- stringr::str_extract_all(rvest::html_text(html_node), "\\(?[0-9,]+\\)?")[[1]]
...
earth.distance <- function(lat1, lon1, lat2, lon2) {
  rad <- pi / 180
  a1 <- lat1 * rad
  a2 <- lon1 * rad
  b1 <- lat2 * rad
