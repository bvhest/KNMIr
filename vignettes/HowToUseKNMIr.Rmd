...
head(list_stations(is_active = FALSE))
```
...
p <- 
  ...
  ggtitle(label = paste("Daily average temperature in", current_year, "vs long-term average"),
...
print(p + 
  ...
  ggtitle(label = "6 day forecasted temperature vs long-term average",
          subtitle = paste("source: KNMI, station: Gilze-Rijen (NL), creation date:",
                           format(Sys.Date(), 
                                  format="%d-%m-%Y"), 
                           sep=" "))
...
print(p + 
  ...
  geom_line(data = forecast,
            aes(x = doy, 
                y = gemTemp),
            color = "#0000FF", show.legend = FALSE, size = 2) + 
  ggtitle(label = "6 and 14 day forecasted temperature vs long-term average",
...
plot_climate_diagram(data = weather_data,
                     column = "gemTemp",
                     startYear = 1981,
                     endYear = 2010,
